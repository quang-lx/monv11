<template>
    <div>
        <el-checkbox v-model="is_select_all" @change="handleCheckAllChange" class="mb-3">
            {{ $t('role.label.select all') }}
        </el-checkbox>



        <el-table ref="permissionTable" :data="groupPermissions" style="width: 100%">
            <el-table-column :label="$t('common.stt')" width="75" :index="indexMethod" type="index">

            </el-table-column>

            <el-table-column property="group_name" :label="$t('role.label.group name')">
                <template slot-scope="scope">
                    <span>{{ scope.row.group_name }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('role.label.function')">
                <template slot-scope="scope">
                    <div class="d-flex flex-row">
                        <div class="  pr-5" v-for="(permission, pIndex) in scope.row.permissions" :key="'p-' + pIndex">
                            ...
                            <el-switch v-model="permissions[permission.name]" active-color="#4B67E2" :active-value="1"
                                :inactive-value="0" inactive-color="#C8E1FF">
                            </el-switch>
