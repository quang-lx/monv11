...
use Modules\Admin\Repositories\Eloquent\EloquentServiceIndexRepository;
...
class ServiceIndexExport implements FromView, WithEvents
{
    protected $request;
    ...
    {
        $repo = new EloquentServiceIndexRepository(new ServiceIndex);
        $query = $repo->queryGetServiceIndex($this->request);

        $list_data = ServiceIndexTransformer::collection($query->get());
